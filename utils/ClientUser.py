...
from disnake.ext.commands import AutoShardedBot, ExtensionNotLoaded, ExtensionAlreadyLoaded, CommandSyncFlags, Context
...
class ClientUser(AutoShardedBot):
    ...
    async def loadNode(self):
        ...
        for node in data:
            try:
                node_obj = await self.nodeClient.create_node(
                    label=node["name"],
                ...
                logger.error(f"Đã xảy ra sự cố khi kết nối đến máy chủ âm nhạc {node['name']}: {e}")
    ...
    async def on_node_ready(self, node: Node):
        ...
        try:
            ...
            if node not in self.available_nodes:
                ...
                logger.info(f"Đã thêm {node.label} vào danh sách nodes khả dụng")
        except Exception as e:
            logger.error(f"Lỗi khi lưu session key: {e}")
    ...
    async def on_node_unavailable(self, node: Node):
        ...
        try:
            if node in self.available_nodes:
                ...
                logger.info(f"Đã remove {node.label} khỏi danh sách nodes khả dụng")
            
            players = list(node.players.values())
            for player in players:
                try:
                    if hasattr(player, 'playing') and player.playing:
                        await player.stop()
                    
                    if hasattr(player, 'connected') and player.connected:
                        await player.disconnect()
                        
                    await player.destroy()
                    logger.info(f"Đã cleanup player cho guild {player.guild.id}")
                except Exception as player_error:
                    logger.warning(f"Không thể cleanup player {player.guild.id}: {player_error}")
            
            await self.nodeClient.remove_node(node)
            logger.info(f"Đã remove node {node.label} khỏi pool")
            
        except Exception as e:
            logger.error(f"Lỗi khi xử lý node unavailable {node.label}: {e}")
        
        if len(self.available_nodes) == 0:
            self.logger.warning("⚠️ Đã hết máy chủ âm nhạc khả dụng, bot có thể sẽ không chạy được nhạc")
            self.loop.create_task(self.reconnect_nodes())

    async def reconnect_nodes(self):
        """Tự động kết nối lại các nodes sau khi mất kết nối"""
        await asyncio.sleep(30)
        logger.info("🔄 Đang thử kết nối lại các nodes...")
        try:
            await self.loadNode()
        except Exception as e:
            logger.error(f"Lỗi khi reconnect nodes: {e}")
    ...
    async def on_ready(self):
        ...
        if not self.connect_node_task.done():
            await self.connect_node_task
            
        logger.info(f"✅ Đã kết nối {len(self.available_nodes)}/{len(data)} nodes Lavalink")

    async def on_slash_command_error(self, inter, error):
        from utils.error import parse_error

        language = await self.database.cached_databases.get_language(inter.guild.id) if inter.guild else "vi"
        original_error = getattr(error, "original", error)
        error_text = parse_error(inter, original_error, language=language)

        if not error_text:
            if isinstance(original_error, asyncio.TimeoutError):
                logger.warning(f"Slash command timeout: {repr(original_error)}")
                error_text = "❌ Yêu cầu bị timeout. Hãy thử lại sau vài giây."
            else:
                logger.error(f"Unhandled slash command error: {repr(original_error)}")
                error_text = f"❌ Đã có lỗi xảy ra: {type(original_error).__name__}"

        try:
            if inter.response.is_done():
                await inter.edit_original_response(content=error_text)
            else:
    ...
    async def on_command_error(self, ctx: Context, error):
        ...
        original_error = getattr(error, "original", error)
        language = await self.database.cached_databases.get_language(ctx.guild.id) if ctx.guild else "vi"
        error_text = parse_error(ctx, original_error, language=language)

        if not error_text:
            logger.error(f"Unhandled command error: {repr(original_error)}")
            error_text = f"❌ Đã có lỗi xảy ra: {type(original_error).__name__}"
            ...
            try:
                ...
                for player in players:
                    try:
                        ...
                        logger.warning(f"Lỗi khi cleanup player {player.guild.id}: {e}")
            except Exception as e:
                logger.warning(f"Lỗi khi cleanup node {node.label}: {e}")
